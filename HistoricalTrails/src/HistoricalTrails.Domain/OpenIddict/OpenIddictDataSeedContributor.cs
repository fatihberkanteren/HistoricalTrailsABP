...
namespace HistoricalTrails.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("HistoricalTrails") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "HistoricalTrails", DisplayName = "HistoricalTrails API", Resources = { "HistoricalTrails" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["HistoricalTrails_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["HistoricalTrails_Web:RootUrl"]!.EnsureEndsWith('/');

            /* HistoricalTrails_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["HistoricalTrails_Web:ClientSecret"] ?? "1q2w3e*",
